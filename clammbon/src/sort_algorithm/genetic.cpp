...
unsigned int genetic::assess(std::vector<int> const& arr) const
{
...
std::vector<std::vector<int>> genetic::select_parent(std::vector<int>& top10)
{
...
void genetic::cross_over(std::vector<std::vector<int>> const& parent_gene)
{
...
genetic::genetic(question_raw_data const& data, compared_type const& comp)
...
std::vector<std::vector<std::vector<point_type>>> genetic::operator() ()
{
    auto const split_total = data_.split_num.first * data_.split_num.second;
    ...
    return {}; //ごめん，どこが答えか分からなかったから空白返す．
}
